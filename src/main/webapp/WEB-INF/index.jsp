...
<body style="background: #000; font-family: Rockwell Extra Bold,Rockwell Bold,monospace; ">
    <div style="background: #000; padding: 15px;">
        <div style="background: #000; color: #66ff00; padding: 10px; margin-left: 40%; width: 20%; border: 2px activeborder groove; height: 300px; word-wrap: break-word;">
            <request>
                <headertext><spring:message code="${pageHeader}" text="${defaulttext}${pageHeader}"/></headertext><br>
                <options>
                    <c:forEach var="myVar" items="${myItems}">
                        <c:if test="${myVar.showOptions}">
                            <option style="word-wrap: break-word;" command="${myVar.command}" order="${myVar.order}" callback="${myVar.url}/${ussdSession.nextPage}" display="${myVar.display}">${myVar.command}. <spring:message code="${myVar.code}" text="${myVar.value}"/></option>
                        </c:if>
                    </c:forEach>
                </options>
            </request>
